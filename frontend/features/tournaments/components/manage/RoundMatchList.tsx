...
import { getMatchSources, type MatchSources } from "../competition/bracket/bracket-presentation";
...
function MatchRounds({
  ...
  sources?: Map<string, MatchSources>;
}) {
...
function DoubleEliminationView({
  ...
  const { t } = useLocale();
  const sources = getMatchSources(matches);
        ...
        <div className="flex flex-wrap gap-3">
          {finals.map((match) => (
            <BracketMatchCard
              ...
              sources={sources.get(match.id)}
              label={
                match.activationCondition ? t("bracket.resetFinal") : t("competition.grandFinal")
              }
              linkLabels={linkLabels(match)}
              onSelect={onSelectMatch}
            />
...
export default function RoundMatchList({
  bracket,
  onSelectMatch,
}: {
  bracket: RoundBracket;
