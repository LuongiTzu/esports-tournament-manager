...
export default function TournamentAdminOverrideDialog({
  tournamentName,
  open,
  working,
  onClose,
  ...
  return (
    <dialog
      ...
      className="m-auto w-[min(92vw,34rem)] rounded-2xl border border-line bg-surface-elevated p-0 text-ink shadow-[var(--shadow-elevated)] backdrop:bg-overlay"
    >
      <form
        onSubmit={(event) => {
          event.preventDefault();
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div>
            <span className="grid size-11 place-items-center rounded-xl bg-pending/12 text-pending">
              <ShieldWarningIcon size={22} weight="duotone" />
            </span>
            <h2 className="mt-4 text-xl font-black">
              {t("admin.tournaments.overrideTitle")}
            </h2>
          </div>
          <button
            type="button"
        ...
        <p className="mt-3 text-sm leading-6 text-ink-muted">
          {t("admin.tournaments.overrideDescription")} “{tournamentName}”.
        </p>
        <label className="mt-5 block">
          ...
          <textarea
            ...
            placeholder={t("admin.tournaments.overridePlaceholder")}
          ...
          <span className="mt-1 flex justify-between gap-3 text-xs text-ink-faint">
            <span>{t("admin.tournaments.overrideHint")}</span>
        ...
        <div className="mt-6 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
          ...
          <button
            type="submit"
            disabled={
              working || trimmedReason.length < 10 || trimmedReason.length > 500
            }
